...
glm::vec3 Mat_Phong::CalculateRadiance(IntersectionData& intersectionData, World& world)
{
	...
	for (const auto& light : *intersectionData.lights) {
		...
		//std::cout << "Point: " << glm::to_string(intersectionData.point) << ", Incoming: " << glm::to_string(intersectionData.incoming) << "\n";
	...
	/*	totalDiffuse += light->color * this->diffuseColor * (glm::dot(intersectionData.incoming, intersectionData.normal));
		totalSpecular += light->color * this->specularColor * std::pow(glm::dot(intersectionData.reflection, intersectionData.viewDir), shiny_exp);*/
	}
